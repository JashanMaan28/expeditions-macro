...
- Rebuilt the interface as one quiet control surface. Structure now comes from alpha hairlines with a second inner hairline on every raised surface instead of opaque borders, control density tightens from 40px to 32px, corner radii differentiate by role (6 controls, 10 surfaces, 12 overlays), and WPF's default scrollbars become 6px overlay thumbs. The UI font moves from Fredoka to Segoe UI Variable, with Fredoka kept for the titlebar wordmark and a monospace face for logs, counters, coordinates, and keyboard hints; every counter uses tabular numerals.
- Run supervision gained a real status system. A shared status dot with a stepped pulse drives the rail status block and the current-run card, the run log moves to the monospace face, and the counters become stat tiles that stay muted until a value carries signal. The rail also shows elapsed runtime while a plan runs, mirroring the dashboard clock rather than starting a second timer.
- Reshaped the authoring surfaces. Macro Plan task blocks are two-layer cards with a quiet numbered index tile, loop ranges render as an inset accent-tinted container whose left rail turns warning-toned when the loop repeats forever, and the autosave state sits beside the plan title as a status dot with a short caption. In Placement Setup, the Start Game step became a full-width success boundary rule with a labelled chip so the before/after split reads at a glance, and the zoom cluster moved to a compact monospace chip at the bottom-right of the map.
- Restyled the shell and remaining pages. The titlebar shows page context as a breadcrumb and gives the close button the standard red hover treatment, selected navigation uses a soft accent fill with a 2px accent bar, secondary rail links demote to quiet ghost rows, and the update banner uses an accent left bar instead of a full accent border. Key-binding capture buttons became monospace wells, the recordings list gained monospace metadata and a shared empty-state treatment, and benchmark timings align in the monospace face.
- Raised both palettes to WCAG AA. Every body, caption, and metadata colour now clears 4.5:1 against each surface it is used on in both themes: the faint tone lightens in dark and darkens in light, light-theme muted darkens to keep the three text levels distinct, and accent-filled controls use a deeper strong-accent shade so white labels stay legible while the display accent continues to carry text, icons, rails, and focus rings.
- Rebalanced the main workspace layouts. Dashboard, Macro Plan, and Settings content now sits in centered max-width columns instead of stretching across wide windows. The current-run card pairs a full-width runtime tile with victories and defeats below it, and the run log stretches to fill the remaining card height without ever letting log content resize the card. The failure-ping user ID joined the Discord notifications card, Macro Plan gained a labelled plan picker and name row, and every Settings section shares one two-column form rhythm.
- Added restrained motion. Switching workspaces fades the incoming page in (with a small rise outside the Dashboard, whose pinned live view must not drift), hover states on buttons, navigation items, and list rows fade in over 100ms and out over 150ms, modal scrims fade in beneath the standard dialog enter animation, and the navigation rail animates its width when collapsing or expanding, swapping labels only while it is wide enough to hold them. Every animation is skipped under the Windows reduced-animation preference and during UI snapshot rendering, which now renders deterministically instead of racing storyboard clocks.
- The Dashboard live-view refresh timer now pauses whenever pinning cannot occur, such as on other workspaces, while unpinned, or while a macro dialog is open, instead of polling four times a second for the lifetime of the app.
- The redesign is visual only. No workflow, detector, persistence, or Roblox-input behaviour changed, the pinned Roblox live view keeps its exact 808 by 611 geometry, and placement coordinate mapping, zoom, and pan logic are untouched. `Themes/Base.xaml` and `Themes/Forms.xaml` are split into `Typography`, `Metrics`, and four `Controls.*` dictionaries, all storyboards honour the Windows reduced-animation preference, and `DESIGN.md` records the new values.

### Fixed

- Loop validation messages in the Macro Plan editor now render in the error color. They previously referenced an undefined `DangerBrush` resource and silently fell back to the default text color.
- The Dashboard live view no longer leaves a black band around the pinned Roblox client at display scales above 100%. The view sizes itself to the client's fixed 808 by 611 physical pixels for the current monitor DPI and follows DPI changes when the window moves between monitors.
